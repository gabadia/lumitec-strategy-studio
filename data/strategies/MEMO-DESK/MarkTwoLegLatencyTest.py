...
class MarkTwoLegLatencyTest(LumitecBaseStrategy):
    ...
    def __init__(self, config: Config):
        ...
        # High-frequency diagnostics are buffered in memory and written only
        # after a batch resolves or the strategy stops. This keeps file I/O and
        # UI event rendering out of the latency-sensitive signal path.
    ...
    def _evaluate_signal(self, signal_ts: float) -> None:
        ...
        now = time.monotonic()
        age_limit = self.params.max_quote_age_ms / 1000.0

        if not self._quote_a.valid or not self._quote_b.valid:
            return

        if now - self._quote_a.receive_ts > age_limit:
            self._record_event(
                "QUOTE_STALE",
                leg="A",
                symbol=self._symbol_a,
                quote_age_ms=round((now - self._quote_a.receive_ts) * 1000, 3),
            )
            return
        if now - self._quote_b.receive_ts > age_limit:
            self._record_event(
                "QUOTE_STALE",
                leg="B",
                symbol=self._symbol_b,
                quote_age_ms=round((now - self._quote_b.receive_ts) * 1000, 3),
            )
            return

        skew = abs(self._quote_a.receive_ts - self._quote_b.receive_ts)
        skew_limit = self.params.max_cross_instrument_skew_ms / 1000.0
        if skew > skew_limit:
            self._record_event(
                "CROSS_INSTRUMENT_SKEW",
                skew_ms=round(skew * 1000, 3),
    ...
    def _initialize_event_log(self) -> None:
        """Prepare a unique JSONL path without writing in the signal path."""
        try:
    ...
    def _record_event(self, record_type: str, **details) -> None:
        """Append a structured diagnostic record in memory; perform no file I/O."""
    ...
    def _flush_event_log(self) -> None:
        """Append buffered records to JSONL after terminal work or at shutdown."""
        if not self._event_log_buffer or self._event_log_path is None:
            return
